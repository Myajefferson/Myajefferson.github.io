...
describe("Test Guru", function () {
  ...
  it("Functions don't have access to each other's scope", function(){
    ...
    function foo(){
      ...
      expect(this.from_yay !== "i'm in foo").to.be.true;
    }
    yay();
    foo();
  });
  ...
  it("We can do goofy stuff with outer scope", function(){
    ...
    yay();
    expect(name === "yippeegreg").to.be.true;
    yay();
    expect(name === "yippeegreggreg").to.be.true;  
    yay();
    expect(name === "yippeegreggreggreg").to.be.true;
